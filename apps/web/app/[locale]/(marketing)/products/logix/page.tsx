  ...
  const capabilities: Capability[] = [
    {
      ...
      desc: isTh
        ? 'การประสานงานเอเจนต์ การใช้เหตุผล และการเรียกใช้เครื่องมืออัตโนมัติ'
        : 'Orchestration, reasoning, and tool use across autonomous agents.',
    },
    {
      Icon: Library,
      title: isTh ? 'ระบบความรู้แบบรวมศูนย์' : 'Unified Knowledge System',
      desc: isTh
        ? 'รวม Wiki, Vectors และ Graph ไว้ในระบบเดียว'
        : 'Wiki, Vectors, and Graph unified into a single knowledge layer.',
    },
    {
      Icon: Brain,
      title: isTh ? 'หน่วยความจำสามชั้น' : 'Triple Memory',
      desc: isTh
        ? 'Episodic + Semantic + Procedural เพื่อความต่อเนื่องของบริบท'
    ...
    },
    {
      ...
      title: 'MCP Hub',
      desc: isTh
        ? 'โปรโตคอลมาตรฐานสำหรับการเชื่อมต่อเครื่องมือ'
        : 'A standard protocol hub for integrating tools.',
    },
    {
      Icon: Sparkles,
      title: isTh ? 'Skills + Marketplace' : 'Skills + Marketplace',
      desc: isTh
        ? 'ระบบอัตโนมัติที่ผู้ใช้สร้างเองและแชร์ได้'
    ...
    },
    {
      ...
      title: 'Personas',
      desc: isTh
        ? 'กำหนดบุคลิก AI ได้ตามแต่ละกรณีการใช้งาน'
        : 'Configurable AI personas tailored per use case.',
    },
    {
      Icon: Server,
      title: isTh ? 'ตัวจัดการโครงสร้างพื้นฐาน' : 'Infrastructure Manager',
      desc: isTh
        ? 'GUI สำหรับจัดการ vLLM / Ollama / SGLang'
        : 'A GUI for managing vLLM, Ollama, and SGLang.',
    },
    {
      Icon: Rocket,
      title: isTh ? 'ติดตั้งในคลิกเดียว' : 'One-Click Deployment',
      desc: isTh
        ? 'แคตตาล็อกแอป AI พร้อมรองรับ repo ที่กำหนดเอง'
        : 'A catalog of AI apps plus support for custom repos.',
    },
    {
      Icon: KeyRound,
      title: isTh ? 'ระบบ API Token' : 'API Token System',
      desc: isTh
        ? 'ให้แอปภายนอกเชื่อมต่อผ่าน API ได้อย่างปลอดภัย'
        : 'Let external applications integrate securely via API.',
    },
    {
      Icon: Network,
      title: isTh ? 'การกำหนดเส้นทาง LLM แบบไฮบริด' : 'Hybrid LLM Routing',
      desc: isTh
        ? 'กำหนดเส้นทางระหว่าง Cloud และ On-Premise อย่างโปร่งใส'
      ...
      <section className="section-padding bg-base-100">
        <div className="container-custom">
          <div className="max-w-3xl">
            <p className="eyebrow">{isTh ? 'ผลิตภัณฑ์ · Logix' : 'Product · Logix'}</p>
            <div className="rule-accent" />
            <h1 className="display-heading text-4xl md:text-5xl lg:text-6xl mt-6">
              {isTh ? 'แพลตฟอร์ม AI อธิปไตยของคุณ' : 'Your Sovereign AI Platform'}
            </h1>
          ...
          <div className="mt-10 grid md:grid-cols-2 gap-8">
            <div className="card-surface p-8 bg-base-100">
              ...
              <ul className="mt-5 space-y-2 text-sm text-base-content/70">
                <li className="flex items-start gap-2">
                  ...
                  {isTh ? 'ข้อมูลไม่ออกจากองค์กร' : 'Data never leaves your organization'}
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  {isTh ? 'รองรับ vLLM / Ollama / SGLang' : 'vLLM / Ollama / SGLang serving'}
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  {isTh ? 'ควบคุมโครงสร้างพื้นฐานเต็มรูปแบบ' : 'Full infrastructure control'}
                </li>
              </ul>
            </div>
            ...
            <div className="card-surface p-8 bg-base-100">
              ...
              <ul className="mt-5 space-y-2 text-sm text-base-content/70">
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  {isTh ? 'ขยายขนาดได้ยืดหยุ่น' : 'Elastic scaling'}
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  {isTh ? 'เริ่มใช้งานได้เร็ว' : 'Fast time to value'}
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  {isTh ? 'กำหนดเส้นทางแบบไฮบริดร่วมกับ On-Premise' : 'Hybrid routing with on-premise'}
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {capabilities.map((c) => (
              <div key={c.title} className="card-surface p-6">
                ...
                <p className="mt-2 text-sm text-base-content/70 leading-relaxed">{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
